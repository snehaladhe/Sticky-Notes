...
function addNewTask(newTask, taskDoDate) {
            ...
            closeButton.addEventListener('click', function(e) {
                ...
                syllbus.splice(other.length-1 - $(".note").index(n), 1);
                var jsonother = JSON.stringify(other);
                localStorage.setItem("other", jsonother);
...
function loadpage()
{
    ...
    t = localStorage.getItem('other');
    if ( t !== null) {
        //var note_container = document.querySelector('.note_container');
        other = JSON.parse(localStorage.getItem('other'));

        for (var i = 0; i < other.length; i++)
            addNewTask(other[i].newTask, other[i].taskDoDate);
    }
}
