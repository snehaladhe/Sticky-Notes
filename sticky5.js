...
function addNewTask(newTask, taskDoDate) {
            ...
            closeButton.addEventListener('click', function(e) {
                ...
                syllbus.splice(edu.length-1 - $(".note").index(n), 1);
                var jsonedu = JSON.stringify(edu);
                localStorage.setItem("other", jsonedu);
...
document.querySelector('form').addEventListener('submit', function(e) { 
    ...
    if (localStorage.getItem('edu') !== null){
        edu = JSON.parse(localStorage.getItem('edu'));       
    }
    // enters localstorage
    edu.push({
        newTask: newTask,
    ...
    var jsonedu = JSON.stringify(edu);
    localStorage.setItem("edu", jsonedu);
    setForm();//to clear
    e.preventDefault();
    return false;
});
...
function loadpage()
{
    ...
    t = localStorage.getItem('edu');
    if ( t !== null) {
        //var note_container = document.querySelector('.note_container');
        edu = JSON.parse(localStorage.getItem('edu'));

        for (var i = 0; i < edu.length; i++)
            addNewTask(edu[i].newTask, edu[i].taskDoDate);
    }
}
